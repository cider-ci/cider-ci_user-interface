...
describe "Functions for managing branches_commits" do
  ...
  describe "descendant functions" do
    describe "with_descendants for @commit2" do
      before :each do
        @res = ActiveRecord::Base.connection.exec_query(
          %[SELECT with_descendants('#{@commit2.id}')]
        ).rows
      end
    ...
    describe "is_descendant " do
      it "returns true if the second argument is a descendant of the first" do
        @res = ActiveRecord::Base.connection.exec_query(
          %[SELECT is_descendant('#{@commit2.id}', '#{@commit3.id}')]
        ).rows
      ...
      it "returns false if the second argument is the same as the first" do
        @res = ActiveRecord::Base.connection.exec_query(
          %[SELECT is_descendant('#{@commit2.id}', '#{@commit2.id}')]
        ).rows
      ...
      it "returns false if the second argument is not
        ...
        @res = ActiveRecord::Base.connection.exec_query(
          %[SELECT is_descendant('#{@commit2.id}', '#{@commit1.id}')]
        ).rows
        expect(@res[0][0]).to be == false
      end
    end
  ...
  describe "ancestor functions" do
    describe "with_ancestors for @commit2" do
      before :each do
        @res = ActiveRecord::Base.connection.exec_query(
          %[SELECT with_ancestors('#{@commit2.id}')]
        ).rows
      end
    ...
    describe "is_ancestor " do
      it "returns true if the second argument is a ancestor of the first" do
        @res = ActiveRecord::Base.connection.exec_query(
          %[SELECT is_ancestor('#{@commit2.id}', '#{@commit1.id}')]
        ).rows
        expect(@res[0][0]).to be == true
      end
      ...
      it "returns false if the second argument is the same as the first" do
        @res = ActiveRecord::Base.connection.exec_query(
          %[SELECT is_ancestor('#{@commit2.id}', '#{@commit2.id}')]
        ).rows
        expect(@res[0][0]).to be == false
      end
      ...
      it "returns false if the second argument is not a ancestor of the first,
        ...
        @res = ActiveRecord::Base.connection.exec_query(
          %[SELECT is_ancestor('#{@commit2.id}', '#{@commit3.id}')]
        ).rows
        expect(@res[0][0]).to be == false
      end
    end
    ...
    describe "add_fast_forward_ancestors_to_branches_commits for commit2" do
      before :each do
        ActiveRecord::Base.connection.execute \
          " SELECT add_fast_forward_ancestors_to_branches_commits('#{@branch.id}',
          '#{@commit2.id}') ".squish
        @res = ActiveRecord::Base.connection.exec_query(
          "SELECT * from branches_commits"
        ).rows
  ...
  describe "update_branches_commits function" do
    ...
    describe "setting 1 (and then fast forward to 3) " do
      ...
      describe "fast forward from 1 to 3" do
        before :each do
          ActiveRecord::Base.connection.execute(
            %[ SELECT update_branches_commits('#{@branch.id}',
