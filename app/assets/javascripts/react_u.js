...
(function(document, window, React) {
  ...
  var mountReactComponents = function() {
    ...
    for (var i = 0; i < nodes.length; ++i) {
      ...
      var component = React.renderComponent(constructor(props), node);
      ReactComponents.push(component);
      // setInterval(function () {
      //   reload(component, node);
      // }, 1000);
      setInterval(reload.bind(null, component, node), 1000);
    }
  };
  
  function reload(component, node) {
    $.getJSON('executions.json', function(data){
      var newProps = { executions: data };
      component.replaceProps(newProps, function callback() {
