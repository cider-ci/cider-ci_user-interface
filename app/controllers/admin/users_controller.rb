...
class Admin::UsersController < AdminController
  ...
  def create
    ...
    redirect_to admin_users_path, flash: {success: "The user has been created"}
  end

  def destroy
    User.find(params[:id]).destroy
    redirect_to admin_users_path, flash: {success: "The user has been deleted"}
  end
  ...
  def index
    ...
    if search_term = user_text_search_filter
      ...
      @users= \
        @users.joins("LEFT OUTER JOIN email_addresses ON email_addresses.user_id = users.id") \
