...
class Admin::DefinitionsController < AdminController
  ...
  def create
    ...
    Fun.wrap_exception_with_redirect self, rescue_path do
      @specification = Specification.find_or_create_by_data! \
        YAML.load(params[:specification][:data])
      @definition = Definition.create! permited_definition_params(params)
      .merge(specification: @specification)
      redirect_to admin_definitions_path,
                  flash: { successes: ['A new Definition has been created.'] }
    end
  end
  ...
  def destroy
    Fun.wrap_exception_with_redirect self, admin_definitions_path do
      ...
      redirect_to admin_definitions_path, flash:
        { successes: [%(The definition "#{@definition}" has been deleted.)] }
    end
  end
  ...
  def update
    ...
    Fun.wrap_exception_with_redirect self, rescue_path do
      ActiveRecord::Base.transaction do
        ...
        redirect_to admin_definitions_path,
                    flash: { successes: [%(The definition has been updated.)] }
      end
    end
  end
